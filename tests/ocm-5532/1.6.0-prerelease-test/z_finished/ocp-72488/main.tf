...
locals {
  ...
  cluster_name = "tr-tf-72488"
  vpc_id = "vpc-0809354a86cc73bf9"
  subnet_public_id = "subnet-09f0bdcb176f1ff09"
}
...
locals {
  ...
  operator_role_prefix = "tr-tf-72488"
...
resource "rhcs_cluster_rosa_hcp" "rosa_hcp_cluster" {
  ...
  proxy = {
    additional_trust_bundle = data.local_file.additional_trust_bundle.content
    http_proxy = "http://${aws_instance.tf-proxy.private_ip}:8080"
    https_proxy = "https://${aws_instance.tf-proxy.private_ip}:8080"
    no_proxy = "quay.io"
  }
}
