...
locals {
  ...
  operator_role_prefix = "tr-tf-72485"
...
resource "aws_kms_key" "second_cluster_kms_key" {
  description             = "BYOK Test Key for API automation 2"
  tags                    = { red-hat = true }
  deletion_window_in_days = 7
}

resource "aws_kms_key_policy" "second_cluster_kms_key_policy" {
  key_id = aws_kms_key.second_cluster_kms_key.id
  policy = jsonencode({
    # Id = var.kms_name
    Statement = [
      {
...
resource "rhcs_cluster_rosa_hcp" "rosa_hcp_cluster" {
  ...
  kms_key_arn = aws_kms_key.second_cluster_kms_key.arn
}
